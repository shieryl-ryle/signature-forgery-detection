...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        # basic parameters
        parser.add_argument('--dataroot', default='results/gan/gan_signdata_kaggle/gan_ips/testB', help='path to images (should have subfolders trainA, trainB, valA, valB, etc)')
    ...
    def parse(self):
        ...
        if torch.cuda.is_available() and len(opt.gpu_ids) > 0 and opt.gpu_ids[0] >= 0:
            torch.cuda.set_device(opt.gpu_ids[0])
        else:
            opt.gpu_ids = []  # Set to empty list if no GPU is available or CUDA is not found
